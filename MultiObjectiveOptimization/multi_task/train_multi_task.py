    ...
    for epoch in tqdm(range(NUM_EPOCHS)):
        ...
        for batch in train_loader:
            ...
            if 'mgda' in params['algorithm']:
                ...
                if approximate_norm_solution:
                    ...
                    for t in tasks:
                        ...
                        loss_data[t] = loss.data[0] if loss.data.dim() != 0 else loss.data
                        loss.backward()
                        grads[t] = []
                        if list_rep:
                    ...
                    for t in tasks:
                        ...
                        loss = loss_fn[t](out_t, labels[t])
                        # print("Losses:", loss, loss.shape, loss.data.dim(), loss.data)
                        loss_data[t] = loss.data[0] if loss.data.dim() != 0 else loss.data
                        loss.backward()
                        grads[t] = []
            ...
            for i, t in enumerate(tasks):
                ...
                loss_data[t] = loss_t.data[0] if loss.data.dim() != 0 else loss.data
                if i > 0:
            ...
            writer.add_scalar('training_loss', loss.data[0] if loss.data.dim() != 0 else loss.data, n_iter)
            for t in tasks:
        ...
        for batch_val in val_loader:
            ...
            for t in tasks:
                ...
                tot_loss['all'] += loss_t.data[0] if loss.data.dim() != 0 else loss.data
                tot_loss[t] += loss_t.data[0] if loss.data.dim() != 0 else loss.data
                metric[t].update(out_t_val, labels_val[t])
            num_val_batches+=1

        for t in tasks:
            writer.add_scalar('validation_loss_{}'.format(t), tot_loss[t]/num_val_batches, n_iter)
            print('\tvalidation loss {}: {}'.format(t, tot_loss[t]/num_val_batches))
        ...
        print('validation loss: {}'.format(tot_loss['all']/len(val_dst)))
